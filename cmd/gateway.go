...
func runGateway() {
	...
	if pgStores != nil && pgStores.KB != nil && mediaStore != nil {
		...
		if ep := resolveEmbeddingProvider(pgStores.Providers, providerRegistry, pgStores.SystemConfigs); ep != nil {
			kbEmbProvider = ep
		}
	...
	pairingMethods, heartbeatMethods, chatMethods := registerAllMethods(server, agentRouter, pgStores.Sessions, pgStores.Cron, pgStores.Pairing, cfg, cfgPath, workspace, dataDir, msgBus, execApprovalMgr, pgStores.Agents, pgStores.Skills, pgStores.ConfigSecrets, pgStores.Teams, contextFileInterceptor, logTee, pgStores.Heartbeats, pgStores.ConfigPermissions, pgStores.SystemConfigs, pgStores.Tenants, browserMgr, scraperCookieStore, pgStores.News, pgStores.Notifications)
	...
	if pgStores.ChannelInstances != nil {
		...
		if projectManagerForShutdown != nil && projectStoreForChannels != nil {
			instanceLoader.RegisterFactory(channels.TypeTelegram, telegram.FactoryWithAllStores(pgStores.Agents, pgStores.ConfigPermissions, pgStores.Teams, projectStoreForChannels, projectManagerForShutdown, pgStores.News, pgStores.Social, socialManager, pgStores.PendingMessages))
		} else {
			instanceLoader.RegisterFactory(channels.TypeTelegram, telegram.FactoryWithAllStores(pgStores.Agents, pgStores.ConfigPermissions, pgStores.Teams, nil, nil, pgStores.News, pgStores.Social, socialManager, pgStores.PendingMessages))
		}
	...
	pgStores.Cron.SetOnJob(makeCronJobHandler(sched, msgBus, cfg, channelMgr, pgStores.Sessions, scheduleHandler))
